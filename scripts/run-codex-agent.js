...
(async () => {
  try {
    let todos;
    if (process.env.FAKE_TODOS) {
      todos = process.env.FAKE_TODOS;
    } else if (hasRg()) {
      // Use ripgrep when available (faster, respects .gitignore by default but we include hidden files)
      todos = execSync('rg --hidden --no-ignore --glob "!.git" --glob "!node_modules" -n "TODO" | head -n 20', { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
    } else {
      todos = execSync('grep -R --exclude-dir=node_modules --exclude-dir=.git "TODO" -n . | head -n 20', { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
    }
    ...
    if (OPENAI_API_KEY) {
      if (process.env.MOCK_OPENAI) {
        ...
        try {
          ...
          const out = resp.output && resp.output[0] && resp.output[0].content ? resp.output[0].content.map(c=>c.text || '').join('\n') : '';
          aiSummary = out || '';
          console.log('AI summary generated.');
        } catch (err) {
          if (err && err.message && err.message.includes('401')) {
            console.error('OpenAI call failed: 401 Unauthorized — invalid or expired OPENAI_API_KEY. Set a valid key or use MOCK_OPENAI for tests.');
          } else {
    ...
    if (DRY_RUN) {
      ...
      execSync(`echo "${CODEX_GH_PAT}" | gh auth login --with-token`, { stdio: 'inherit' });
      execSync(`gh issue create --repo ldshawver/luxverified-video --title "${title}" --body-file ${tmp}`, { stdio: 'inherit' });
